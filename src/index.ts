const game = document.getElementById('gameTable') as HTMLElement
const overview = document.getElementById('overviewTable') as HTMLElement
const bednickaInput = document.getElementById('bednickaInput') as HTMLInputElement
const allRevealedNumber = document.getElementById('allRevealedTiles') as HTMLElement
...
const VALUES_NUMBER: number = originValues.length

function resetBednicka() {
  const item = document.getElementById('bednickaTile') as HTMLInputElement
  if (item.children.length > 0) item.removeChild(item.children[0])
  ...
  if (bednickaInput.value !== "" && bednickaInput.valueAsNumber >= +bednickaInput.min && bednickaInput.valueAsNumber <= +bednickaInput.max) {
    ...
    const input = document.getElementById('bednickaInput') as HTMLInputElement
...
function getAllRevealedNumber() {
  let sum = 0
  for (let i = 0; i < game.children.length; i++) {
    if (game.children[i].classList.contains('active')) sum++
  }
  return sum
}
...
function getSumOfAllNonActiveTiles() {
  ...
  for (let i = 0; i < game.children.length; i++) {
    if (!game.children[i].classList.contains('active')) {
      const item = game.children[i] as HTMLElement
...
function offerFormula(count: number | string, value: number = 0) {
...
function calculateOffer() {
  ...
  if (numberOfLeftTiles === 20) offerFormula(numberOfLeftTiles, 0.75)
  else if (numberOfLeftTiles === 15) offerFormula(numberOfLeftTiles, 0.79)
  else if (numberOfLeftTiles === 11) offerFormula(numberOfLeftTiles, 0.83)
  else if (numberOfLeftTiles === 8) offerFormula(numberOfLeftTiles, 0.86)
  else if (numberOfLeftTiles === 6) offerFormula(numberOfLeftTiles, 0.89)
  else if (numberOfLeftTiles === 5) offerFormula(numberOfLeftTiles, 0.91)
  else if (numberOfLeftTiles === 4) offerFormula(numberOfLeftTiles, 0.93)
  else if (numberOfLeftTiles === 3) offerFormula(numberOfLeftTiles, 0.94)
  else if (numberOfLeftTiles === 2) offerFormula(numberOfLeftTiles, 0.95)
  else if (numberOfLeftTiles === 1) offerFormula("To co je v tvé bedničce pausechamp !")
  else {
    offerFormula("... OC")
  }
}
  ...
  for (let i = 0; i < VALUES_NUMBER; i++) {
    ...
    div.addEventListener('click', () => {
      ...
      for (let k = 0; k < VALUES_NUMBER; k++) {
        const item = overview.children[k] as HTMLElement
